...
这里注意运算顺序,数字1 + 2 + 3等于6 数字再与字符串进行运算转换为字符串
'5' + 3等于'53' '53' + 4 等于'534'
...
```javascript
if ('http://github.com/daledu.jpg'.match('.github')) {
    console.log('a gifhub file');
} else {
    console.log('not a gifhub file');
}
```
由于.匹配的是任意
...
像这种很大的数,比如1234678 * 10^5 会留一位来保存幂数,但是a没有空间来保存幂数
